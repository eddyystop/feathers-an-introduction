...
function user() {
  ...
  const { softDelete, setCreatedAt, setUpdatedAt, iff, isProvider, remove } = commonHooks;
  const isMethod = method => hook => hook.method === method;
  
  userService.before({
    all: iff(!isMethod('find'), softDelete()), // new
    create: [
      validateSchema(userSchema(), Ajv), authHooks.hashPassword(), setCreatedAt(), setUpdatedAt()
    ]});
  userService.after({
    all: iff(isProvider('external') && !isMethod('find'), remove('password')),
  });
}
